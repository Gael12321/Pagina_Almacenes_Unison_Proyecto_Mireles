...
class FormularioMaterial(forms.ModelForm):
    class Meta:
        model = Material
        fields = ['nombre_articulo', 'precio_unitario', 'categoria', 'cantidad', 'unidades_o_paquetes', 'cantidad_por_paquete', 'imagen', 'descripcion', 'umbral', 'Origen']

widgets = {
    'unidades_o_paquetes': forms.Select(attrs={'id': 'id_unidades_o_paquetes'}),
    'cantidad_por_paquete': forms.NumberInput(attrs={'id': 'id_cantidad_por_paquete'}),}
